...
impl<T: Event> ChannelHandle<T> {
    fn from_state(kind: ChannelKind, state: &ChannelState) -> Result<Self> {
        match kind {
...
impl EventBus {
    ...
    /// # }
    /// ```
    pub fn subscribe_mpsc<T: Event>(&self, capacity: usize) -> Result<mpsc::Receiver<Arc<T>>> {
        let capacity = validate_capacity(capacity)?;
    ...
    /// # }
    /// ```
    pub fn subscribe_watch<T: Event>(&self, initial: T) -> Result<watch::Receiver<Arc<T>>> {
    ...
    /// # }
    /// ```
    pub fn publish_arc<T: Event>(&self, event: Arc<T>) -> Result<usize> {
        let sender =
    ...
    /// # }
    /// ```
    pub fn publish_mpsc<T: Event>(&self, event: T) -> Result<()> {
    ...
    /// # }
    /// ```
    pub fn publish_mpsc_arc<T: Event>(&self, event: Arc<T>) -> Result<()> {
    ...
    /// # }
    /// ```
    pub fn publish_watch<T: Event>(&self, event: T) -> Result<()> {
    ...
    /// # }
    /// ```
    pub fn publish_watch_arc<T: Event>(&self, event: Arc<T>) -> Result<()> {
        let arc = event;
    ...
    fn get_or_create_mpsc<T: Event>(&self, capacity: usize) -> Result<mpsc::Sender<Arc<T>>> {
        let capacity = validate_capacity(capacity)?;
        let id = TypeId::of::<T>();
        let existing = {
            let mut channels = self.channels.write();
    ...
    fn take_mpsc_receiver<T: Event>(&self, capacity: usize) -> Result<mpsc::Receiver<Arc<T>>> {
        let capacity = validate_capacity(capacity)?;
        let id = TypeId::of::<T>();
        let existing = {
            let mut channels = self.channels.write();
...
fn validate_capacity(capacity: usize) -> Result<usize> {
