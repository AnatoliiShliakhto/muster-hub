...
fn check_expiry(license: &SignedLicense) -> Result<(), LicenseError> {
    let now = SystemTime::now()
        ...
        .map_err(|e| {
            LicenseError::internal()
                .with_message("failed to get current system time")
    ...
    if now < license.data.issued {
        Err(LicenseError::internal()
            .with_context("system clock is set before license issuance date"))?;
    }

    if now > license.data.expires {
        Err(LicenseError::expired()
            .with_context_fn(|| format!("license expired at `{}`", license.data.expires)))?;
    }
...
fn validate_hardware(constraint: &MachineConstraint) -> Result<(), LicenseError> {
    match constraint {
        ...
        MachineConstraint::Threshold { ids, min_matches } => {
            // Current machine: compound -> 3 parts
            ...
            Err(LicenseError::hardware_mismatch().with_context_fn(|| format!(
...
fn verify_signature(license: &SignedLicense, public_key: &[u8; 32]) -> Result<(), LicenseError> {
    ...
    let data_bytes =
        postcard::to_stdvec(&license.data).with_context("binary serialization failed")?;
