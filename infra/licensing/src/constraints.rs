...
//! - Permissions may restrict reading of certain IDs.
//!
...
fn build_component(component: HWIDComponent) -> Result<String, LicenseError> {
    IdBuilder::new(Encryption::SHA256)
        ...
        .build(KEY)
        .map_err(|e| LicenseError::machine_id_generation().with_context_fn(|| e.to_string()))
}
...
pub fn parse_machine_id_compound(s: &str) -> Result<Vec<String>, LicenseError> {
    let s = s.strip_prefix(PREFIX).ok_or_else(|| {
        LicenseError::machine_id_generation()
            .with_context_fn(|| format!("Invalid machine id prefix `{s}`"))
    })?;

    let parts: Vec<&str> = s.split(SEP).collect();
    if parts.len() != 3 || parts.iter().any(|p| p.is_empty()) {
        Err(
            LicenseError::machine_id_generation().with_context("Expected 3 parts separated by '|'")
        )?;
    }
