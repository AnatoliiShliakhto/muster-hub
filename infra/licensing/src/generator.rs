...
pub fn generate_secret() -> Result<[u8; 32], LicenseError> {
    ...
    fill(&mut secret).map_err(|e| {
        LicenseError::generation().with_context_fn(|| format!("failed to generate secret ({e})"))
    })?;
    Ok(secret)
}
...
pub fn generate_keypair() -> Result<(SigningKey, VerifyingKey), LicenseError> {
    ...
    fill(&mut seed).map_err(|e| {
        LicenseError::generation().with_context_fn(|| format!("failed to generate seed ({e})"))
    })?;
    ...
    let now = SystemTime::now()
        ...
        .map_err(|e| {
            LicenseError::internal()
                .with_message("Failed to calculate current time")
