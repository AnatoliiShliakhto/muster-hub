...
impl MigrationRunner {
    ...
    async fn apply_migration(&self, migration: &Migration) -> Result<(), DatabaseError> {
        ...
        let _ = self
            ...
            .check()
            .with_context_fn(|| {
                format!("migration failed at `{}`->`{}`", &migration.slice_key, &migration.version)
            })?;
    ...
    async fn is_system_ready(&self) -> Result<bool, DatabaseError> {
        let mut response = self
            ...
            .await
            .with_context("checking if system is ready")?
            .check()
            .with_context("failed to check system status")?;
    ...
    async fn get_migrations_map(
        ...
        let entries = self
            ...
            .await
            .with_context("transport error while loading migrations")?
            .check()
            .with_context("database failed to execute migration query")?
            .take::<Vec<AppliedMigration>>(0)
            .with_context("failed to map database rows to AppliedMigration struct")?;
    ...
    pub(crate) async fn sync_permissions(&self) -> Result<(), DatabaseError> {
        ...
        self.db
            ...
            .await
            .with_context("failed to execute `fn::sync_permissions` query")? // Catch network/syntax
            .check()
            .with_context("internal error during `fn::sync_permissions` execution")?;
...
fn ensure_checksum_match(migration: &Migration, existing: &str) -> Result<(), DatabaseError> {
    if existing != migration.checksum {
        Err(DatabaseError::migration().with_context_fn(|| {
            format!("checksum mismatch for `{}`->`{}`", migration.slice_key, migration.version)
        }))?;
    }
    Ok(())
}
