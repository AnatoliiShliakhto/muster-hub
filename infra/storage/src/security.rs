...
fn normalize_relative(path: &Path) -> Result<PathBuf, StorageError> {
    ...
    for c in path.components() {
        match c {
            ...
            Component::ParentDir => {
                if !out.pop() {
                    Err(StorageError::path_traversal_attempt().with_context_fn(|| {
                        format!("path attempted to escape sandbox via '..' `{}`", path.display())
                    }))?;
                }
            },
            Component::RootDir | Component::Prefix(_) => {
                Err(StorageError::path_traversal_attempt().with_context_fn(|| {
                    format!("path contains absolute path components `{}`", path.display())
                }))?;
            },
        }
    }
...
pub(crate) fn resolve_path(root: &Path, path: impl AsRef<Path>) -> Result<PathBuf, StorageError> {
    ...
    if path.is_absolute() {
        Err(StorageError::path_traversal_attempt().with_context_fn(|| {
            format!("absolute paths are not allowed in sandbox `{}`", path.display())
        }))?;
    }
    ...
    match joined.canonicalize() {
        ...
        Err(e) => Err(StorageError::from(e)
            .with_context_fn(|| format!("failed to resolve path `{}`", joined.display()))),
    }
}
...
pub(crate) fn resolve_sharding(
    ...
    let filename = path.file_name().and_then(|s| s.to_str()).ok_or_else(|| {
        StorageError::file_not_found()
            .with_context_fn(|| format!("target must be a file `{}`", path.display()))
    })?;
...
fn validate_canonical(root: &Path, canonical: PathBuf) -> Result<PathBuf, StorageError> {
    if canonical.starts_with(root) {
        ...
        Err(StorageError::path_traversal_attempt().with_context_fn(|| {
            format!("path attempted to escape sandbox via '..' sequences `{}`", canonical.display())
        }))
    }
}
...
fn validate_path(root: &Path, joined: &Path) -> Result<PathBuf, StorageError> {
    if !joined.starts_with(root) {
        Err(StorageError::path_traversal_attempt().with_context_fn(|| {
            format!("path is outside sandbox boundaries `{}`", joined.display())
        }))?;
    }
        ...
        if path.exists() {
            return match path.canonicalize() {
                ...
                Ok(canonical) => {
                    Err(StorageError::path_traversal_attempt().with_context_fn(|| {
                        format!(
                            "path attempted to escape sandbox via symlink `{}`",
                            canonical.display()
                        )
                    }))
                },
                Err(e) => Err(StorageError::from(e).with_context_fn(|| {
                    format!("failed to verify parent directory `{}`", path.display())
                })),
            };
        }
    ...
    Err(StorageError::path_traversal_attempt().with_context_fn(|| {
        format!("path attempted to escape sandbox via '..' sequences `{}`", joined.display())
    }))
}
