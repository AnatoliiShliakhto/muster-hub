...
pub fn resolve_path(root: &Path, path: impl AsRef<Path>) -> Result<PathBuf> {
...
pub fn resolve_sharding(root: &Path, ns: Option<&str>, path: impl AsRef<Path>) -> Result<PathBuf> {
    let path = path.as_ref();
...
fn validate_canonical(root: &Path, canonical: PathBuf) -> Result<PathBuf> {
...
fn validate_path(root: &Path, joined: &Path) -> Result<PathBuf> {
