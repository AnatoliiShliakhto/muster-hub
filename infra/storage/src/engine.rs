...
impl Compression {
    ...
    fn decompress(self, data: &[u8]) -> Result<Vec<u8>, StorageError> {
        match self {
            ...
            Self::Lz4 => {
                lz4_flex::decompress_size_prepended(data).with_context("Lz4 decompression failed")
            },
        }
    }
}
...
///
/// ```rust
/// use mhub_storage::{Storage, Compression};
/// use mhub_storage::error::StorageError;
///
/// #[tokio::main]
...
impl Storage {
    ...
    pub(crate) async fn read_internal(
        ...
        let data = match fs::read(&resolved).await {
            ...
            Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
                Err(StorageError::file_not_found()
                    .with_context_fn(|| resolved.display().to_string()))?
            },
            Err(err) => Err(StorageError::from(err)
                .with_context_fn(|| format!("failed to read '{}'", resolved.display())))?,
        };
    ...
    pub(crate) async fn write_internal(
        ...
        if let Some(parent) = resolved.parent() {
            fs::create_dir_all(parent).await.with_context_fn(|| {
                format!("failed to create shards for `{}`", resolved.display())
            })?;
        }
...

        {
            let mut file = fs::OpenOptions::new()
                ...
                .open(&temp)
                .await
                .with_context_fn(|| format!("temp creation failed `{}`", temp.display()))?;
            file.write_all(&final_data).await.with_context("write failed")?;
            file.sync_all().await.with_context("hardware sync failed")?;
        }

        if let Err(err) = fs::rename(&temp, &resolved).await {
            if err.kind() == std::io::ErrorKind::AlreadyExists {
                fs::remove_file(&resolved).await.with_context_fn(|| {
                    format!("failed to replace existing file `{}`", resolved.display())
                })?;
                fs::rename(&temp, &resolved).await.with_context_fn(|| {
                    format!("atomic swap failed `{}` -> `{}`", temp.display(), resolved.display())
                })?;
            } else {
                Err(StorageError::from(err).with_context_fn(|| {
                    format!("atomic swap failed `{}` -> `{}`", temp.display(), resolved.display())
                }))?;
            }
        }
    ...
    pub(crate) async fn delete_internal(
        ...
        match fs::remove_file(&resolved).await {
            ...
            Err(err) => Err(StorageError::from(err)
                .with_context_fn(|| format!("failed to delete `{}`", resolved.display())))?,
        }
    ...
    pub async fn metadata(
        ...
        match fs::metadata(&resolved).await {
            ...
            Err(err) => Err(StorageError::from(err)
                .with_context_fn(|| format!("failed to get metadata `{}`", resolved.display()))),
        }
    }
