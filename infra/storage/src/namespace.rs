...
impl TryFrom<&str> for NamespaceName {
    ...
    fn try_from(value: &str) -> Result<Self, StorageError> {
        ...
        if name.is_empty() {
            Err(StorageError::path_traversal_attempt().with_context("namespace cannot be empty"))?;
        }

        if !name.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
            Err(StorageError::path_traversal_attempt()
                .with_context_fn(|| format!("namespace contains illegal characters `{name}`")))?;
        }
