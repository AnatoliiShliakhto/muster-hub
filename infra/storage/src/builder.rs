...
impl StorageBuilder<WithRoot> {
    ...
    /// - The root directory doesn't exist, and `create` is false.
    ...
    pub async fn connect(self) -> Result<Storage, StorageError> {
        ...
        if self.config.create {
            fs::create_dir_all(root).await.with_context_fn(|| {
                format!("failed to bootstrap storage root `{}`", root.display())
            })?;
        ...
        let canonical = fs::canonicalize(root)
            .await
            .with_context_fn(|| format!("failed to resolve storage root `{}`", root.display()))?;
