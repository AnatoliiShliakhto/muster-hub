...
fn test_vault_ext_roundtrip() {
    ...
    let sealed = config.seal_local_with_ctx(&vault, b"").expect("Sealing failed");
    ...
    let unsealed: SecureConfig =
        vault.unseal_with_ctx(&sealed, &"SecureConfig").expect("Unsealing failed");
    assert_eq!(config, unsealed);

    let sealed = config.seal_fleet_tagged(&vault).expect("Sealing failed");
    let unsealed: SecureConfig = vault.unseal_tagged(&sealed).expect("Unsealing failed");
    assert_eq!(config, unsealed);
    ...
    let sealed = config.seal_fleet_with_ctx(&vault, &b"").expect("Sealing failed");
    let unsealed: SecureConfig = vault.unseal(&sealed).expect("Unsealing failed");
    assert_eq!(config, unsealed);
}
...
fn test_algorithm_agility_cha_cha() {
    let vault = Vault::<ChaCha>::builder().with_derived_keys("key", "salt", "id").build().unwrap();
