...
impl<C: VaultCipher> VaultBuilder<C> {
    ...
    pub fn derived_keys(
        ...
        hk.expand(b"v1_fleet:", &mut fleet).map_err(|e| {
            VaultError::encryption()
                .with_message("HKDF expansion failed for fleet key")
        ...
        hk.expand(&info, &mut local).map_err(|e| {
            VaultError::encryption()
                .with_message("HKDF expansion failed for local key")
                .with_context_fn(|| e.to_string())
        })?;
...
impl<C: VaultCipher> VaultBuilder<C, WithKeys> {
    ...
    fn init_cipher(key: &[u8; 32], context: &'static str) -> Result<C, VaultError> {
        let key = Key::<C>::try_from(&key[..]).map_err(|e| {
            VaultError::invalid_configuration().with_context_fn(|| format!("{context}: {e}"))
        })?;
        Ok(C::new(&key))
    }
}
