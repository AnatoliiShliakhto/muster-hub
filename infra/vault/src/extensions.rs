...
use crate::{
    Fleet, Local, PayloadKind, ProtectedPayload, Result, SecurityVault,
};
use serde::Serialize;
...
pub trait VaultExt: Serialize {
    ...
    ///
    /// # Errors
    /// * See [`Self::seal_local_tagged`] for failure modes.
    fn seal_fleet_tagged<C>(
        &self,
        vault: &SecurityVault<C>,
    ) -> Result<ProtectedPayload<Fleet, C>>
    where
