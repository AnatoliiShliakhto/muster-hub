...
use crate::{PayloadKind, ProtectedPayload, VaultCipher};
...
pub fn serialize<S, K, C>(p: &ProtectedPayload<K, C>, s: S) -> Result<S::Ok, S::Error>
where
    S: Serializer,
    K: PayloadKind<C>,
    C: VaultCipher,
...
/// container for later unsealing via the [`SecurityVault`].
///
/// # Errors
/// Returns a deserialization error if the input data is not a valid byte sequence.
pub fn deserialize<'de, D, K, C>(d: D) -> Result<ProtectedPayload<K, C>, D::Error>
where
    D: Deserializer<'de>,
    K: PayloadKind<C>,
    C: VaultCipher,
