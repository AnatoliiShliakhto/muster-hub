...
use crate::types::{
    Aes, FLAG_COMPRESSED, HEADER_LEN, NONCE_LEN, PAYLOAD_VERSION_V1, PayloadKind, TAG_LEN,
    VaultCipher,
};
use crate::{ProtectedPayload, Result, VaultBuilder, VaultError, VaultErrorExt, VaultSerde};
    ...
    C: VaultCipher,
{
    ...
    pub fn seal<K, T>(&self, data: &T) -> Result<ProtectedPayload<K, C>>
    where
        K: PayloadKind<C>,
        T: VaultSerde,
    {
    ...
    /// * [`VaultError::PostcardSerialization`] If the decrypted bytes cannot be parsed.
    /// * [`VaultError::Decompression`] If the LZ4 stream is corrupt.
    pub fn unseal<K, T>(&self, payload: impl AsRef<[u8]>) -> Result<T>
    where
        K: PayloadKind<C>,
        C: VaultCipher,
        T: VaultSerde,
    ...
    /// * [`VaultError::PostcardSerialization`] If the decrypted bytes cannot be parsed.
    /// * [`VaultError::Decompression`] If the LZ4 stream is corrupt.
    pub fn unseal_local<T>(&self, payload: impl AsRef<[u8]>) -> Result<T>
    where
        T: VaultSerde,
    {
    ...
    /// * [`VaultError::PostcardSerialization`] If the decrypted bytes cannot be parsed.
    /// * [`VaultError::Decompression`] If the LZ4 stream is corrupt.
    pub fn unseal_fleet<T>(&self, payload: impl AsRef<[u8]>) -> Result<T>
    where
        T: VaultSerde,
    {
    ...
    /// * [`VaultError::Decryption`] If the context, key, or data is invalid.
    /// * [`VaultError::Decompression`] If the LZ4 stream is corrupt.
    pub fn unseal_local_bytes(&self, payload: impl AsRef<[u8]>, context: &[u8]) -> Result<Vec<u8>> {
    ...
    /// * [`VaultError::Decryption`] If the context, key, or data is invalid.
    /// * [`VaultError::Decompression`] If the LZ4 stream is corrupt.
    pub fn unseal_fleet_bytes(&self, payload: impl AsRef<[u8]>, context: &[u8]) -> Result<Vec<u8>> {
    ...
    fn encrypt_internal(cipher: &C, data: &[u8], aad: &[u8], compress: bool) -> Result<Vec<u8>> {
    ...
    fn decrypt_internal(cipher: &C, blob: &[u8], aad: &[u8]) -> Result<Vec<u8>> {
