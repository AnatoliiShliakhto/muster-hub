    ...
    C: VaultCipher,
{
    ...
    fn next_nonce() -> Result<Nonce<C>, VaultError> {
        ...
        fill(&mut nonce).map_err(|e| {
            VaultError::internal()
                .with_message("system RNG unavailable for nonce generation")
    ...
    where
        ...
        T: VaultSerde,
    {
        let bytes = postcard::to_stdvec(data).with_context("postcard encoding failed")?;
        ...
        T: VaultSerde,
    {
        ...
        postcard::from_bytes(&bytes).with_context("postcard decoding failed")
    }
    ...
    fn encrypt_internal(
        ...
        let tag = cipher.encrypt_inout_detached(&nonce, aad, in_out).map_err(|e| {
            VaultError::encryption()
                .with_message("AEAD encryption failed")
                .with_context_fn(|| e.to_string())
        })?;
    ...
    fn decrypt_internal(cipher: &C, blob: &[u8], aad: &[u8]) -> Result<Vec<u8>, VaultError> {
        if blob.len() < (HEADER_LEN + NONCE_LEN + TAG_LEN) {
            Err(VaultError::invalid_payload().with_context_fn(|| {
                format!(
        ...
        if version != PAYLOAD_VERSION_V1 {
            Err(VaultError::invalid_payload()
                .with_context_fn(|| format!("unsupported payload version `{version}`")))?;
        }
        ...
        let nonce = nonce_slice.try_into().map_err(|e| {
            VaultError::decryption().with_context_fn(|| format!("invalid nonce: {e}"))
        })?;

        let tag = tag_slice.try_into().map_err(|e| {
            VaultError::decryption().with_context_fn(|| format!("invalid tag: {e}"))
        })?;

        let mut buf = ciphertext.to_vec();
        let in_out = InOutBuf::from(&mut buf[..]);

        cipher.decrypt_inout_detached(&nonce, aad, in_out, &tag).map_err(|e| {
            VaultError::decryption().with_context_fn(|| format!("AEAD decryption failed: {e}"))
        })?;
        ...
        if compressed {
            buf = lz4_flex::decompress_size_prepended(&buf).map_err(|e| {
                VaultError::decompression()
                    .with_context_fn(|| format!("LZ4 decompression failed: {e}"))
            })?;
        }
...
mod tests {
    ...
    fn test_nonce_sequence() {
        let n1 = Vault::<ChaCha>::next_nonce().expect("nonce generation should succeed");
        let n2 = Vault::<ChaCha>::next_nonce().expect("nonce generation should succeed");
