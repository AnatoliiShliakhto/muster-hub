    ...
    pub fn init(self) -> Result<Logger, LoggerError> {
        ...
        if self.config.console {
            let console_layer = layer()
                .compact()
                .with_ansi(true)
                .fmt_fields(FilteredFields {
                    ignored: &["details", "backtrace", "target", "code", "kind", "context"],
                })
                .boxed();
        ...
        let guard = if let Some(path) = self.config.path {
            fs::create_dir_all(&path)
                .with_context_fn(|| format!("failed to create path `{}`", path.display()))?;
            ...
            let file_appender = RollingFileAppender::builder()
                ...
                .build(path)
                .map_err(|e| LoggerError::appender().with_context_fn(|| e.to_string()))?;
        ...
        if layers.is_empty() {
            return Err(LoggerError::invalid_configuration()
                .with_context("no logging layers enabled")
                .with_help("Enable console, file output, or OpenTelemetry."));
        }
...
struct FilteringVisitor<V> {
    inner: V,
    ignored: &'static [&'static str],
}

impl<V: Visit> Visit for FilteringVisitor<V> {
...
fn validate_config(config: &LoggerConfig, name: &str) -> Result<(), LoggerError> {
    if name.trim().is_empty() {
        return Err(
            LoggerError::invalid_configuration().with_context("Logger name cannot be empty")
        );
    }

    if config.max_files == 0 {
        return Err(LoggerError::invalid_configuration()
            .with_context("`max_files` must be greater than zero"));
    }
        ...
        |filter| {
            builder.parse(filter).map_err(|e| {
                LoggerError::invalid_configuration()
                    .with_context(format!("invalid env filter `{filter}`: {e}"))
            })
        },
    )
}
...
mod tests {
    ...
    fn test_logger_builder_configuration() -> Result<(), LoggerError> {
        let tmp_dir = tempdir().with_context("failed to create temp dir")?;
    ...
    fn test_file_logging_setup() -> Result<(), LoggerError> {
        ...
        let entries = fs::read_dir(&log_dir).with_context("failed to read log directory")?;
