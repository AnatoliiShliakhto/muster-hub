...
impl LoggerBuilder {
    /// Creates a new builder with the given application name.
    ///
    /// The `app_name` is used as a prefix for log files (e.g., `server.log`).
...
impl Logger {
    ...
    ///     .unwrap();
    /// ```    
    pub fn builder(app_name: impl Into<String>) -> LoggerBuilder {
        LoggerBuilder::new(app_name)
    }
}
...
mod tests {
    ...
    fn test_logger_builder_initial_state() {
        ...
        assert_eq!(builder.app_name, "test-app");
