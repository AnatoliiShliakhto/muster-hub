...
/// # Ok::<(), mhub_logger::error::LoggerError>(())
/// ```
pub fn init_otlp_tracer(
    ...
    if service_name.trim().is_empty() {
        return Err(
            LoggerError::invalid_configuration().with_context("service_name cannot be empty")
        );
    }
    ...
    let exporter = opentelemetry_otlp::SpanExporter::builder()
        .with_tonic()
        .build()
        .with_context("failed to build OTLP span exporter")?;
