...
use crate::error::{RuntimeError, RuntimeErrorExt};
...
/// # Errors
///
/// Returns an [`RuntimeError`] if the Tokio runtime cannot be created, typically due to
...
pub fn build_runtime(stack_size: usize) -> Result<Runtime, RuntimeError> {
...
/// # Errors
///
/// Returns an [`RuntimeError`] if the Tokio runtime cannot be created. Common causes include:
...
pub fn build_runtime_with_config(config: &RuntimeConfig) -> Result<Runtime, RuntimeError> {
    ...
    builder
        .build()
        .with_context_fn(|| format!("Failed to initialize runtime with config: {config:?}"))
}
...
/// * [`get_global_runtime()`] for accessing the global shared runtime
pub fn build_service_runtime() -> Result<Runtime, RuntimeError> {
    let config = RuntimeConfig::default();
    info!(
