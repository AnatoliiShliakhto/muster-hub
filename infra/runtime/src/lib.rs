...
/// # Errors
///
/// Returns an [`Error::Internal`] if the Tokio runtime cannot be created, typically
...
pub fn build_runtime(stack_size: usize) -> Result<Runtime> {
    build_runtime_with_config(
        &RuntimeConfig::default().with_stack_size(stack_size),
    )
}
...
/// # Errors
///
/// Returns an [`Error::Internal`] if the Tokio runtime cannot be created. Common causes include:
...
pub fn get_global_runtime() -> &'static Runtime {
    GLOBAL_RUNTIME.get_or_init(|| {
        build_service_runtime().expect(
            "CRITICAL: Failed to initialize global infrastructure runtime",
        )
    })
}
...
mod tests {
    ...
    fn test_stack_size_validation() {
        ...
        let config =
            RuntimeConfig::default().with_stack_size(100 * 1024 * 1024);
