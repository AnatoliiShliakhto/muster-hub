...
/// 256 is usually enough for domain event-bus in a vertical slice.
            ...
            ChannelKind::Broadcast { .. } => {
                let sender =
                    state.sender.downcast_ref::<broadcast::Sender<Arc<T>>>().ok_or_else(|| {
                        EventBusError::type_mismatch().with_context(std::any::type_name::<T>())
                    })?;
...
impl EventBus {
    ...
    ///
    /// # #[tokio::main]
    /// # async fn main() -> Result<(), mhub_event_bus::error::EventBusError> {
    ...
    /// struct Tick(u64);
    ///
    /// # fn main() -> Result<(), mhub_event_bus::error::EventBusError> {
    /// let bus = EventBus::new();
    ...
    pub fn subscribe_with_capacity<T: Event>(
        ...
        match sender {
            ...
            ChannelHandle::Watch(_) => {
                Err(EventBusError::type_mismatch().with_context(std::any::type_name::<T>()))
            },
        }
    }
    ...
    pub fn publish_mpsc_arc<T: Event>(&self, event: Arc<T>) -> Result<(), EventBusError> {
        ...
        sender
            .try_send(event)
            .map_err(|e| EventBusError::channel_full().with_context_fn(|| e.to_string()))
    }
    ...
    fn ensure_channel<T: Event>(
        ...
        let mut watch_initial = if matches!(kind, ChannelKind::Watch) {
            Some(watch_initial.ok_or_else(|| {
                EventBusError::type_mismatch().with_context(format!(
                    "watch channel `{}` requires an initial value",
                    std::any::type_name::<T>()
                ))
            })?)
        } else {
        ...
        let existing_handle =
            {
                ...
                let result = match (existing.kind, kind) {
                    ...
                    _ => Err(EventBusError::channel_kind_mismatch().with_context_fn(|| format!(
                ...
                Some(existing) => match existing.kind {
                    ChannelKind::Mpsc { .. } => {
                        match existing.sender.downcast_mut::<MpscChannel<T>>() {
                            ...
                            None => Some(Err(EventBusError::type_mismatch()
                                .with_context(std::any::type_name::<T>()))),
                        }
                    },
                    other => {
                        Some(Err(EventBusError::channel_kind_mismatch().with_context_fn(|| {
                            format!(
                    ...
                    ChannelKind::Mpsc { .. } => {
                        match existing.sender.downcast_mut::<MpscChannel<T>>() {
                            Some(chan) => {
                                ...
                                if chan.taken {
                                    Some(Err(EventBusError::channel_kind_mismatch()
                                        .with_context_fn(|| {
                                            format!(
                                                "MPSC receiver already taken `{}`",
                                                std::any::type_name::<T>()
                                            )
                                        })))
                                } else {
                                    chan.taken = true;
                                    Some(Err(EventBusError::channel_not_found()
                                        .with_context(std::any::type_name::<T>())))
                                }
                            },
                            None => Some(Err(EventBusError::type_mismatch()
...
fn validate_capacity(capacity: usize) -> Result<usize, EventBusError> {
    if capacity < MIN_CAPACITY {
        Err(EventBusError::invalid_capacity()
            .with_context_fn(|| format!("capacity must be >= {MIN_CAPACITY}")))?;
    }
    Ok(capacity)
}
