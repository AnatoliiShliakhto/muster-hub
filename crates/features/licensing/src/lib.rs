...
impl SignedLicense {
    ...
    pub fn encode_bin(&self) -> Result<Vec<u8>> {
    ...
    pub fn validate(&self, public_key: &[u8; 32]) -> Result<()> {
        validator::validate_license(self, public_key)
    }
    ...
    pub fn secure_clear(mut self) {
        use zeroize::Zeroize;
        self.data.constraint.zeroize();
        self.data.customer.zeroize();
        self.data.salt.zeroize();
        self.data.issued_at.zeroize();
        self.data.expires_at.zeroize();
