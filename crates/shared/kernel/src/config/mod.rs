...
/// Custom error type for application configuration loading and parsing.
#[mhub_error::error]
pub enum ConfigError {
    /// Failure during the discovery, reading, or parsing of configuration files/environment variables.
    #[error(
        message = "Application configuration failure",
    ...
    T: DeserializeOwned,
{
    ...
    let builder = Config::builder()
        .add_source(File::from(effective_path.as_path()).required(false))  // Now required since we checked existence
        .add_source(
