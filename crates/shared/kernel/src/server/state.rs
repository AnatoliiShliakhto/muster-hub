...
/// Errors related to the global API state, slice registration, and initialization lifecycle.
#[mhub_error::error]
pub enum ApiStateError {
    /// Failure during the startup sequence or dependency injection of a subsystem.
    #[error(
        message = "Subsystem initialization failure",
        code = 503,
        kind = "STATE_INITIALIZATION_ERROR"
    )]
    Initializing,

    /// A requested feature slice or module has not been registered or is unavailable.
    #[error(message = "Requested slice not found", code = 404, kind = "STATE_SLICE_NOT_FOUND")]
    MissingSlice,
}
...
impl ApiState {
    ...
    pub fn try_get_slice<T: FeatureSlice>(&self) -> Result<&T, ApiStateError> {
        self.get_slice::<T>()
            .ok_or_else(|| ApiStateError::missing_slice().with_context(std::any::type_name::<T>()))
    }
...
impl ApiStateBuilder {
    ...
    pub fn build(self) -> Result<ApiState, ApiStateError> {
        let config = self.config.ok_or_else(|| {
            ApiStateError::initializing()
                .with_context("config not provided")
                .with_help("Use `ApiStateBuilder::config()` to set it")
        })?;
        let database = self.database.ok_or_else(|| {
            ApiStateError::initializing()
                .with_context("config not provided")
                .with_help("Use `ApiStateBuilder::db()` to set it")
        })?;
