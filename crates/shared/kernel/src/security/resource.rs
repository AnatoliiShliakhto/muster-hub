...
pub enum ResourceGuardError {
    #[error(message = "Invalid resource ID", code = 400, kind = "AUDIT_RESOURCE_VALIDATION")]
    Validation,
}
...
impl ResourceGuard {
        ...
        T: AsRef<str>,
    {
        ...
        if let Some((table, _)) = id_ref.split_once(':') {
            if table != table_ref {
                Err(ResourceGuardError::validation()
                    .with_context_fn(|| format!("expected `{table_ref}`, got `{table}`")))?;
            }
