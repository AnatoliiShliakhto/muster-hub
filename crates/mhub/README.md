...
- `server`: enables kernel server parts and identity/licensing server slices.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let slices = mhub::init(&config, &database)?;
...
- `init` currently wires identity (and licensing when enabled); extend it as new slices are added.
