...
pub fn codegen_migrations() -> Result<(), AppError> {
    ...
    fs::write(&manifest_path, manifest_content)
        .with_context_fn(|| format!("Failed to write manifest to `{}`", manifest_path.display()))?;
...
fn resolve_execution_order(nodes: Vec<MigrationNode>) -> Result<Vec<MigrationNode>, AppError> {
    let node_map: FxHashMap<String, MigrationNode> =
        nodes.into_iter().map(|n| (n.key.clone(), n)).collect();

    // 1. Identification
    let bootstrap_key =
        node_map.values().find(|n| n.is_bootstrap).map(|n| n.key.clone()).ok_or_else(|| {
            AppError::codegen().with_context("Missing crate with `bootstrap = true`")
        })?;
    ...
    for node in node_map.values() {
        ...
        for dep in deps {
            if !node_map.contains_key(&dep) {
                return Err(AppError::codegen().with_context_fn(|| {
                    format!("Crate `{}` depends on unknown crate `{dep}`", node.key)
                }));
            }
    ...
    if sorted.len() != node_map.len() {
        return Err(AppError::codegen()
            .with_context("Circular dependency detected. Graph contains cycles.")
            .with_help("Consider adding explicit dependencies to avoid cycles."));
    }
...
fn discover_nodes() -> Result<Vec<MigrationNode>, AppError> {
    let mut nodes = Vec::new();

    let mut scan = |dir: &str, kind: NodeKind| -> Result<(), AppError> {
        for crate_info in get_workspace_crates(dir)? {
            ...
            let raw_key = crate_info
                ...
                .file_name()
                .and_then(|n| n.to_str())
                .ok_or_else(|| AppError::codegen().with_context("Invalid crate path"))?;
    ...
    if nodes.iter().filter(|n| n.is_bootstrap).count() > 1 {
        return Err(AppError::codegen()
            .with_context("Multiple bootstrap crates defined")
            .with_help("Check `bootstrap = true` in the `Cargo.toml`"));
    }
...
fn render_manifest(nodes: &[MigrationNode], root: &Path) -> Result<String, AppError> {
...
fn render_permissions(w: &mut String, nodes: &[MigrationNode]) -> Result<(), AppError> {
...
fn load_migration_config(path: &Path) -> Result<MigrationConfig, AppError> {
    let toml_path = path.join("Cargo.toml");
    let content = fs::read_to_string(&toml_path)
        .with_context_fn(|| format!("Failed to read Cargo.toml at `{}`", toml_path.display()))?;

    let meta: PackageMetadata = toml_edit::de::from_str(&content).map_err(|e| {
        AppError::parse()
            .with_message(e.to_string())
            .with_context_fn(|| format!("Failed to parse Cargo.toml at `{}`", toml_path.display()))
    })?;
...
fn ensure_parent_dir(path: &Path) -> Result<(), AppError> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).with_context_fn(|| {
...
fn read_surql_files(dir: &Path) -> Result<Vec<PathBuf>, AppError> {
    ...
    for entry in fs::read_dir(dir)
        .with_context_fn(|| format!("Failed to read directory `{}`", dir.display()))?
    {
        let path = entry
            .with_context_fn(|| format!("Failed to read directory entry for `{}`", dir.display()))?
            .path();
...
fn validate_sql_content(path: &Path) -> Result<(), AppError> {
    let s = fs::read_to_string(path)
        .with_context_fn(|| format!("Failed to read SQL file `{}`", path.display()))?
        .to_lowercase();
    if s.contains("begin transaction") || s.contains("commit transaction") {
        return Err(AppError::codegen().with_context_fn(|| {
            format!(
                "Manual transaction control prohibited in `{}`.\n\
                The migration runner handles transactions.",
                path.display()
            )
        }));
    }
...
fn extract_version(path: &Path) -> Result<String, AppError> {
    let stem = path.file_stem().and_then(|s| s.to_str()).ok_or_else(|| {
        AppError::codegen().with_context_fn(|| format!("Invalid filename `{}`", path.display()))
    })?;

    // Expect format: 0000-name
    if !stem.chars().next().map_or(false, |c| c.is_ascii_digit()) || !stem.contains('-') {
        Err(AppError::codegen()
            .with_context_fn(|| format!("Invalid version format `{stem}`"))
            .with_help("Expected format `0000-name.surql`"))?;
    }
    Ok(stem.to_owned())
}
...
fn resolve_relative_path(path: &Path, root: &Path) -> Result<String, AppError> {
    let path_str = path
        .to_str()
        .ok_or_else(|| {
            AppError::internal().with_context_fn(|| format!("Invalid path `{}`", path.display()))
        })?
        .replace('\\', "/");
    let root_str = root
        .to_str()
        .ok_or_else(|| {
            AppError::internal()
                .with_context_fn(|| format!("Invalid root path `{}`", root.display()))
        })?
        .replace('\\', "/");

    path_str.strip_prefix(&format!("{root_str}/")).map(|s| format!("../../../../{s}")).ok_or_else(
        || AppError::codegen().with_context_fn(|| format!("Invalid relative path `{path_str}`")),
    )
}

fn calculate_checksum(path: &Path) -> Result<String, AppError> {
    let bytes =
        fs::read(path).with_context_fn(|| format!("Failed to read file `{}`", path.display()))?;
