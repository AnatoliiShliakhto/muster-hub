...
impl ServerBuilder {
    ...
    async fn init_database(&self) -> Result<Database> {
        ...
        let mut builder = Database::builder()
            .with_url(&db_cfg.url)
            .with_session(&db_cfg.namespace, &db_cfg.database);

        if let Some(creds) = &db_cfg.credentials {
            builder = builder.with_auth(&creds.username, &creds.password);
        }

        builder.connect().await.context("Failed to establish database connection")
    }
    ...
    pub async fn build(self) -> Result<Server> {
        ...
        let slices =
            mhub::init(&self.cfg, &db).map_err(|e| anyhow!("Platform bootstrap failed: {e}"))?;

        // 4. Construct State using Functional Folding
        let state = slices
            .into_iter()
            .fold(ApiState::builder().with_config(self.cfg).with_db(db), |builder, slice| {
